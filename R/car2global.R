...
#' @param y				The y coordinates, optional if x is an appropriate structure.
#' @param z				The z coordinates, optional if x is an appropriate structure.
#' @param r,theta,phi	Spherical/polar coordinates, given as for x, y and z.
#' @param perm			Logical: If TRUE the input points 'r' are given as a row matrix (see 'r').
#' @param nonneg		Logical: If TRUE negative values of 'theta' should be added 2*pi.
#' @param lon			The vector of longitude values.
#' @param lat			The vector of latitude values.
#' @param toaxis		Logical: If TRUE the radius to the axis of Earth is to be given.
#' @param digits		The number of digits in the output of \code{DD2DMS}.
...
car2global <- function(pos, origin=c(0,0,0), list.out=FALSE, format=c("dec", "min", "sec")){
	...
	if(is.list(pos)){
		...
		if(any(length(hitspsx)>1,length(hitspsy)>1,length(hitspsz)>1)){
			warning(paste0("More than one set of 'psx*', 'psy*' and 'psz*' values. Selecting '", namespos[hitspsx[1]], "', '", namespos[hitspsy[1]], "' and '", namespos[hitspsz[1]], "'"))
		}
...
global2car <- function(pos, origin=1, list.out=FALSE){
	...
	if(is.list(pos)){
		...
		if(any(length(hitslon)>1,length(hitslat)>1)){
			warning(paste0("More than one set of 'lon*' and 'lat*' values. Selecting '", namespos[hitslon[1]], "' and '", namespos[hitslat[1]], "'"))
		}
